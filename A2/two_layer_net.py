...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    data_loss = -torch.sum(torch.log(correct_probs)) / N
...
def find_best_net(data_dict, get_param_set_fn):
  ...
  for lr, hs, reg, lr_decay in itertools.product(learning_rates, hidden_sizes, regularization_strengths, learning_rate_decays):
