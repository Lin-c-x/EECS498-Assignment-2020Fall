...
def svm_loss_vectorized(W, X, y, reg):
  ...
  row_sum = torch.sum(binary_margins, dim=1)
  binary_margins[torch.arange(num_train), y] -= row_sum
  dW = X.t().mm(binary_margins) / num_train + 2 * reg * W
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def svm_get_search_params():
  ...
  regularization_strengths = [1e-3, 3e-2, 5e-2, 7e-2, 1e-1]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    ...
    # Numerical stability trick (shift scores to avoid large exponents)
    scores -= torch.max(scores)

    # Compute softmax probabilities
    probs = torch.exp(scores) / torch.sum(torch.exp(scores))

    # Compute the loss (negative log probability of the correct class)
    loss += -torch.log(probs[y[i]])

    # Compute the gradient
    for j in range(num_classes):
        dW[:, j] += X[i] * (probs[j] - (1 if j == y[i] else 0))

  # Average loss and gradient over all examples
  loss /= num_train
  dW /= num_train
    
  # Add regularization term to the loss and gradient
  loss += reg * torch.sum(W * W)
  dW += 2 * reg * W
  #############################################################################
  #                          END OF YOUR CODE                                 #
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  loss = -torch.log(probs[torch.arange(num_train), y]).sum()
...
def softmax_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [7e-3, 3e-2,  7e-2, 1e-1, 2e-1]
  regularization_strengths = [1e-3, 5e-3, 7e-3, 1e-2,  5e-2]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
