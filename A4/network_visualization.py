...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  for i in range(max_iter):
      ...
      print('Iteration %d: target score %.3f, max score %.3f'%(i, scores[:, target_y], scores[:, res]))
      if res == target_y:
          break
      scores[:, target_y].backward()
      grad = X_adv.grad.data
      X_adv.data += grad * learning_rate / torch.norm(grad, 2)
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    loss = scores[:, target_y] - (l2_reg * torch.norm(img, 3))
    loss.backward()
    
    with torch.no_grad():
        grad = img.grad
        img += learning_rate * grad / torch.norm(img, 2)
        img.grad.zero_()
