...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(T):
        next_h, cache_t = rnn_step_forward(x[:, i, :], prev_h, Wx, Wh, b)
        prev_h = next_h
        h[:, i, :] = prev_h
...
def rnn_backward(dh, cache):
    ...
    dprev_h_i = torch.zeros_like(dh0)
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
        ...
        if cell_type in ['rnn', 'lstm']:
            ...
            self.feature_extract = FeatureExtractor(pooling=False, device=device, dtype=dtype)
        ...
        nn.init.kaiming_normal(self.temporal_affine.weight)
    ...
    def forward(self, images, captions):
        ...
        ############################################################################
        # Replace "pass" statement with your code
        feature = self.feature_extract.extract_mobilenet_feature(images)
    ...
    def sample(self, images, max_length=15):
        ...
        for i in range(max_length):
            ...
            if self.cell_type == 'rnn':
                next_h = self.net.step_forward(x, prev_h)
            elif self.cell_type == 'lstm':
                next_h, next_c = self.net.step_forward(x, prev_h, prev_c)
            else:
                attn, attn_weights = dot_product_attention(prev_h, A)
                attn_weights_all[:, i] = attn_weights
                next_h, next_c = self.net.step_forward(x, prev_h, prev_c, attn)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b, attn=None, Wattn=None):
    ...
    # Replace "pass" statement with your code
    N, H = prev_h.shape
    a = x.mm(Wx) + prev_h.mm(Wh) + b + (0 if attn is None else attn.mm(Wattn))
...
def dot_product_attention(prev_h, A):
    ...
    attn_scores = torch.matmul(prev_h, A.view(N, H, -1))
    attn_weights = torch.nn.functional.softmax(attn_scores / (H**0.5), dim=-1).view(N, D_a, D_a)
    attn = torch.sum(attn_weights.view(N, 1, D_a, D_a) * A, dim=(-2, -1)).squeeze()
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def attention_forward(x, A, Wx, Wh, Wattn, b):
    ...
    for t in range(T):
        ...
        h_t, c_t = lstm_step_forward(x[:, t, :], h_t, c_t, Wx, Wh, b, attn=attn, Wattn=Wattn)
        h[:, t, :] = h_t
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
