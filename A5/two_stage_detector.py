...
class ProposalModule(nn.Module):
  ...
  def forward(self, features, pos_anchor_coord=None, \
    ...
    out = self.pred_layer(features).view(B, A, -1, H, W)
    conf_scores = out[:, :, :2, :, :]
    offsets = out[:, :, 2:, :, :]
    ...
    if mode == 'train':
        ...
        proposals = GenerateProposal(pos_anchor_coord.reshape(-1, 1, 1, 1, 4), offsets.reshape(-1, 1, 1, 1, 4), method='FasterRCNN')
...
class RPN(nn.Module):
  ...
  def forward(self, images, bboxes, output_mode='loss'):
    ...
    GT_offsets, GT_class, activated_anc_coord, negative_anc_coord = \
        ReferenceOnActivatedAnchors(anchors, bboxes, grid, ious, method='FasterRCNN')
    # Get pos_anchor_idx (also called activated_anc_ind)
    ...
    # vi) Compute pos_conf_scores
    M = conf_scores.shape[0] // 2
    conf_scores = conf_scores[:M, :1]
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7, mode='RPN'):
    ...
    with torch.no_grad():
        ...
        for b in range(B):
            ...
            conf_probs_mask = conf_probs[b] > thresh
            
            # get boxes for nms
            boxes = proposals[b][conf_probs_mask].view(-1, 4)
            # get scores for nms
            scores = conf_probs[b][conf_probs_mask].view(-1)
            ...
            final_conf_probs.append(scores[nms_indices].unsqueeze(1))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    if mode == 'RPN':
...
class TwoStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    '''
    - pos_conf_scores: (M, 1) giving the object classificationscores for the positive anchors
    - proposals: (M, 4) giving the coordiantes of the region proposals for the positive anchors
    - GT_class: (M,) giving the ground-truth category label for the positive anchors.
    - pos_anchor_idx: (M,) giving indices of positive anchors
    - neg_anchor_idx: (M,) giving indices of negative anchors
    - anc_per_image: Torch scalar giving the number of anchors per image.
    '''
    # ii) Perform RoI Align(K, C, output_size[0], output_size[1]) on proposals and meanpool the feature in the spatial dimension. 
    # Get boxes (Tensor[K, 5]) for torchvision.ops.RoIAlign
    boxes = torch.cat((pos_anchor_idx.unsqueeze(1) // anc_per_img, proposals), dim=1)
    roi = torchvision.ops.roi_align(features, boxes, (self.roi_output_w, self.roi_output_h))
    # Meanpool the feature in the spatial dimension.                           
    roi = torch.mean(roi, dim=(2, 3))
    
    # iii) Pass the RoI feature through the region classification layer which gives the class probilities. 
    class_prob = self.cls_layer(roi)
    
    # iv) Compute the cross entropy loss
    cls_loss = torch.nn.functional.cross_entropy(class_prob, GT_class, reduction='sum') / images.shape[0]
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    with torch.no_grad():
        ...
        # Calculate RoI for getting the class probilities through region classification
        roi = torchvision.ops.roi_align(features, final_proposals, (self.roi_output_w, self.roi_output_h))                           
        roi = torch.mean(roi, dim=(2, 3))   # 74 * 1280
        class_probs = torch.argmax(self.cls_layer(roi), dim=1).unsqueeze(1)     # 74 * 1
        
        # Calculate the number of anchors in each images
        split_index = [t.shape[0] for t in final_proposals]
        final_class = torch.split(class_probs, split_index, dim=0)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
