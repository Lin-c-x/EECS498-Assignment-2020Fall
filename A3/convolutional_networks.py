        ...
        for flr in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    x_slice = x_padded[img, :, i * stride:i * stride + HH, j * stride:j * stride + WW]
                    out[img, flr, i, j] = torch.sum(x_slice * w[flr, :, :, :]) + b[flr]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, w, b, conv_param)
        ...
        for flr in range(F):
            ...
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    dw[flr] += dout[img, flr, i, j] * x_padded[img, :, i * stride:i * stride + HH, j * stride:j * stride + WW]
  ...
  def forward(x, pool_param):
    ...
    for img in range(N):
        for channel in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    x_slice = x[img, channel, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
                    out[img, channel, i, j] = torch.max(x_slice)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, pool_param)
  ...
  def backward(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            max_vals, _ = x_pool_region.reshape(N, C, -1).max(dim=2)
            
            mask = (x_pool_region == max_vals.unsqueeze(2).unsqueeze(3))
            
            dx[:, :, h_start:h_end, w_start:w_end] += mask * dout[:, :, i, j].unsqueeze(2).unsqueeze(3)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class ThreeLayerConvNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    conv_relu_pool, crp_cache = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
    
    linear_relu, lr_cache = Linear_ReLU.forward(conv_relu_pool, W2, b2)
    
    scores, l_cache = Linear.forward(linear_relu, W3, b3)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dl, dW3, grads['b3'] = Linear.backward(dscores, l_cache)
    dlr, dW2, grads['b2'] = Linear_ReLU.backward(dl, lr_cache)
    dcrp, dW1, grads['b1'] = Conv_ReLU_Pool.backward(dlr, crp_cache)
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    for i in range(self.num_layers - 1):
        ...
        if weight_scale == 'kaiming':
            self.params[f'W{i + 1}'] = kaiming_initializer(channels[i], num_f, K=3, relu=True, device=device, dtype=dtype)
        else:
            self.params[f'W{i + 1}'] = weight_scale * torch.randn(num_f, channels[i], 3, 3, dtype=dtype, device=device)
        self.params[f'b{i + 1}'] = torch.zeros(num_f, dtype=dtype, device=device)
        
        if self.batchnorm:
            self.params[f'gamma{i + 1}'] = torch.ones(num_f, dtype=dtype, device=device)
            self.params[f'beta{i + 1}'] = torch.zeros(num_f, dtype=dtype, device=device)
    
    
    fc_dim = num_filters[-1] * (H // (2 ** len(max_pools))) * (W // (2 ** len(max_pools)))
    
    if weight_scale == 'kaiming':
        self.params[f'W{self.num_layers}'] = kaiming_initializer(fc_dim, num_classes, relu=False, device=device, dtype=dtype)
    else:
        self.params[f'W{self.num_layers}'] = weight_scale * torch.randn(fc_dim, num_classes, dtype=dtype, device=device)
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
  ...
  if K is None:
    ...
    # Replace "pass" statement with your code
    fan_in = Din
    weight_scale = torch.sqrt(torch.tensor(gain / fan_in, dtype=dtype))
...
class BatchNorm(object):
  ...
  def backward(dout, cache):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    x, x_normalized, gamma, mean, variance, eps = cache['x'], cache['x_normalized'], cache['gamma'], cache['mean'], cache['variance'], cache['eps']
    N, D = dout.shape
    
    dbeta = torch.sum(dout, dim=0)
    dgamma = torch.sum(x_normalized * dout, dim=0)
    dx_normalized = gamma * dout
    
    x_minus_mean = x - mean
    done_divide_sqrt = torch.sum(dx_normalized * x_minus_mean, dim=0)
    
    sqrt_var = torch.sqrt(variance + eps)
    dsqrt_var = -1 / (sqrt_var ** 2) * done_divide_sqrt 
    dvar = 0.5 / sqrt_var * dsqrt_var
    dx_minus_mean1 = 1 / N * 2 * (x_minus_mean) * dvar
    
    dx_minus_mean2 = dx_normalized / sqrt_var
    
    dx_minus_mean = dx_minus_mean1 + dx_minus_mean2
    
    dx1 = dx_minus_mean
    
    dmean = -1 * torch.sum(dx_minus_mean, dim=0)
    dx2 = 1 / N * dmean
  ...
  def backward_alt(dout, cache):
    ...
    dvariance = torch.sum(dx_normalized * (x - mean) * -0.5 * (variance + eps)**(-1.5), dim=0)
    dmean = torch.sum(dx_normalized * -1 / torch.sqrt(variance + eps), dim=0) + dvariance * torch.sum(-2.0 * (x - mean), dim=0) / N
