...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    scores, scores_cache = Linear.forward(h1, self.params['W2'], self.params['b2'])
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    ...
    dh1, grads['W2'], grads['b2'] = Linear.backward(dscores, scores_cache)
    _, grads['W1'], grads['b1'] = Linear_ReLU.backward(dh1, h1_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(1, self.num_layers + 1):
        self.params[f'W{i}'] = torch.randn(all_dims[i - 1], all_dims[i], dtype=dtype, device=device) * weight_scale
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers):
        ...
        layer_input, cache = Linear_ReLU.forward(layer_input, W, b)
        caches.append(cache)
        
        if self.use_dropout:
            layer_input, dropped = Dropout.forward(layer_input, self.dropout_param)
            dropout_caches.append(dropped)
    
    W, b = self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}']
    
    scores, cache = Linear.forward(layer_input, W, b)
    caches.append(cache)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    for i in range(self.num_layers - 1, 0, -1):
        ...
        if self.use_dropout:
            dx = Dropout.backward(dx, dropout_caches.pop())
...
def create_solver_instance(data_dict, dtype, device):
  ...
  solver = Solver(model, data_dict,
                  update_rule=sgd,  # You may need to choose an appropriate update rule
                  optim_config={'learning_rate': 1e-1},
                  lr_decay=0.9,
                  num_epochs=20, batch_size=100,
                  print_every=100,
                  print_acc_every=1,  # Print accuracy every epoch
                  verbose=True,
                  device=device)
  ##############################################################################
  #                             END OF YOUR CODE                               #
...
class Dropout(object):
  ...
  def forward(x, dropout_param):
    ...
    if mode == 'train':
      ...
      ###########################################################################
      # Replace "pass" statement with your code
      mask = (torch.rand_like(x, dtype=x.dtype, device=x.device) < (1 - p)).float()
      out = x * mask / (1 - p)
      ###########################################################################
      #                             END OF YOUR CODE                            #
      ###########################################################################
